...
def process_dataset(_pred_path, _gt_path):
    ...
    # _pred_path = os.path.join(_pred_path,'c0')
    # _gt_path = os.path.join(_gt_path,'c0')
...
if __name__ == "__main__":
    # gt_pth = '/media/fjy/SHARED/data_fjy/SeasonDepth/val/depth/env00/c0/img_00119_c0_1303398474779439us.png'
    # gt_depth = cv2.imread(gt_pth, -1)
    ...
    for s in sorted(slices):
        ...
        camside = os.listdir(_pred_pth)
        for camtype in sorted(camside):
            _pred_pth = os.path.join(_pred_pth, camtype)
            _gt_pth = os.path.join(_gt_pth, camtype)
            xls_path = os.path.join(xls_path, camtype)
