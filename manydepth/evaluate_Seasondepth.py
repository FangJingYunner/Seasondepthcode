...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        # savefilenames = readlines(os.path.join(splits_dir, opt.eval_split, "save_files.txt"))
        ...
        if opt.eval_split == 'cityscapes':
            dataset = datasets.CityscapesEvalDataset(opt.data_path_val, filenames,
                                                     HEIGHT, WIDTH,
                                                     frames_to_load, 4,
                                                     is_train=False)

        elif opt.eval_split == 'SeasonDepth':
            dataset = datasets.SDDataset(opt.data_path_val, filenames,
                                                     encoder_dict['height'], encoder_dict['width'],
                                                     frames_to_load, 4,
                                                     is_train=False)

        else:
            dataset = datasets.KITTIRAWDataset(opt.data_path_val, filenames,
                                               encoder_dict['height'], encoder_dict['width'],
                                               frames_to_load, 4,
                                               is_train=False)
        ...
        depth_decoder = networks.DepthDecoder(encoder.num_ch_enc).to(device)
        ...
        # do inference
        index = 0
        # output_dir = '/media/fjy/SHARED/data_fjy/SeasonDepth'
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                # pred_disp = pred_disp.cpu()[:, 0].numpy().squeeze()
                # # pred_disps.append(pred_disp)
                # pred_disp = pred_disp * 256
                # pred_disp = pred_disp.astype(int)
                # pred_disp = pred_disp.astype('uint16')
                # output_path = os.path.join(output_dir, data[("filepath")][0])
                # # index += 1
                # new_im = Image.fromarray(pred_disp)
                # # new_im.show()
                # s2 = output_path.split('.')
                # output_path = s2[0] + '.png'
                # new_im.save(output_path)
                pred_disp = pred_disp.cpu().numpy().squeeze(1)
                split_disp = [pred_disp[i , ...] for i in range(opt.batch_size)]
                
                for idx in range(len(split_disp)):
                    disp_resized = cv2.resize(split_disp[idx], (1024, 768))
                    depth = STEREO_SCALE_FACTOR / disp_resized
                    depth = np.clip(depth, 0, 80)
                    depth = np.uint16(depth * 256)
                    output_path = os.path.join(opt.eval_season_output_dir, data[("filepath")][idx])
                    s2 = output_path.split('.')
                    output_path = s2[0] + '.png'
                    # save_path = os.path.join(save_dir, "{:010d}.png".format(idx))
                    cv2.imwrite(output_path, depth)
                    
                # pyplot.imshow(pred_disp)
                # misc.imsave(output_path, new_im)
...
'''
    ...
    if opt.save_pred_disps:
        ...
        if len(savefilenames) != len(pred_disps):
            print('number not good')
        
        output_dir = '/media/fjy/SHARED/data_fjy/SeasonDepth/val'
        pred_disps = pred_disps*256
        pred_disps = pred_disps.astype(int)
        pred_disps = pred_disps.astype('uint16')

        for index ,save_filename in enumerate(savefilenames):
            output_path = os.path.join(output_dir,save_filename)
            np.save(output_path, pred_disps[index])
